...
async function getFrontCameraStream() {
  ...
  // Attempt 1: ask the browser nicely for the front/user-facing camera.
  try {
    return await navigator.mediaDevices.getUserMedia({
      video: { ...baseConstraints, facingMode: { ideal: "user" } },
      audio: false,
    });
  } catch (err) {
    console.warn("[PuzzleCam] facingMode:user request failed, trying device list…", err);
  }

  // Attempt 2: enumerate cameras and pick one whose label suggests front/user-facing,
  // explicitly avoiding anything that looks like a rear/back/environment camera.
  try {
    const devices = await navigator.mediaDevices.enumerateDevices();
    const videoInputs = devices.filter((d) => d.kind === "videoinput");
    const isBackLabel = (label) => /back|rear|environment/i.test(label || "");
    const isFrontLabel = (label) => /front|user|face/i.test(label || "");

    const frontDevice =
      videoInputs.find((d) => isFrontLabel(d.label)) ||
      videoInputs.find((d) => !isBackLabel(d.label)) ||
      videoInputs[0];

    if (frontDevice) {
      return await navigator.mediaDevices.getUserMedia({
        video: { ...baseConstraints, deviceId: { exact: frontDevice.deviceId } },
        audio: false,
      });
    }
  } catch (err) {
    console.warn("[PuzzleCam] Device enumeration fallback failed, trying default camera…", err);
  }

  // Attempt 3: last resort — whatever camera the browser gives us.
  return navigator.mediaDevices.getUserMedia({ video: baseConstraints, audio: false });
}
